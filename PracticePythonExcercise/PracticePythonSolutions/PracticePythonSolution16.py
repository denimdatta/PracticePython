...
def get_int(show_prompt="Enter: ", show_text=None, valid_options: list = None):
    ...
    while True:
        try:
            ...
            if valid_options is not None and _input not in valid_options:
                pass
            else:
                return _input
        except ValueError:
...
def medium_pass():
    pass_len = random.randint(2, 3)
    _password = ""
    for _i in range(0, pass_len):
